...
| --- | --- | --- | --- | --- | --- |
| T1 | 项目结构与配置模型 | [ ] | AI | 规划 `code_index/` 包与模块边界；定义配置 JSON 结构（全局 embedding + 项目索引信息 + chunk 参数）；实现路径 hash 与持久化目录结构；在 `pyproject.toml` 增加依赖与 `code-index` 入口；优先用 apply_patch 修改 | 运行 `python -m code_index --help`；执行 `code-index init` 后生成配置文件与目录 |
| T2 | OpenAI API 兼容 embedding 客户端 | [ ] | AI | 使用 `httpx`/标准库实现 `/v1/embeddings` 调用；支持 base_url/api_key/model；实现批量、超时、错误处理；提供单测与 MockTransport | `pytest -q` 通过；手测返回向量长度与数量一致 |
| T3 | Milvus Lite 存储层 | [ ] | AI | 以 `pymilvus` Lite 模式创建 collection；设计 schema（vector + path + line_start + line_end + file_hash）；实现 insert/search/delete；支持按文件删除 | `pytest -q` 通过；手测 insert/search 返回结果 |
